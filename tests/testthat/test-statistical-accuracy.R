...
test_that("p-chart UCL/LCL matches Montgomery formula (p̄=0.10, n=100)", {
  ...
  expect_equal(result$qic_data$cl[1], p_bar,
    tolerance = 1e-6,
    label = "p-chart centerlinje"
  )

  # UCL og LCL konstante (constant n) — verificér første punkt
  expect_equal(result$qic_data$ucl[1], ucl_expected,
    tolerance = 1e-4,
    label = "p-chart UCL"
  )
  expect_equal(result$qic_data$lcl[1], lcl_expected,
    tolerance = 1e-4,
    label = "p-chart LCL"
  )
})
...
test_that("c-chart UCL/LCL matches Montgomery formula (c̄=5)", {
  ...
  result <- bfh_qic(data, x = period, y = count, chart_type = "c")
  ...
  expect_equal(result$qic_data$cl[1], c_bar,
    tolerance = 1e-6,
...
test_that("u-chart UCL/LCL matches Montgomery formula (ū=0.05, n=100)", {
  ...
  expect_equal(result$qic_data$cl[1], u_bar,
    tolerance = 1e-6,
...
test_that("i-chart UCL/LCL matches Montgomery moving-range method", {
  ...
  expect_equal(result$qic_data$cl[1], x_bar,
    tolerance = 1e-6,
...
test_that("freeze-parameter fryser CL efter baseline-perioden", {
  ...
  data <- data.frame(
    period = 1:24,
    value = c(
      9, 10, 11, 10, 9, 11, 10, 9, 11, 10, 9, 11, # baseline: mean ≈ 10
      19, 20, 21, 20, 19, 21, 20, 19, 21, 20, 19, 21
    ) # post: mean ≈ 20
  )

  result_frozen <- bfh_qic(data, x = period, y = value, chart_type = "i", freeze = 12)

  expect_valid_bfh_qic_result(result_frozen)

  # Baseline-mean er 10 (fra første 12 punkter)
  baseline_mean <- mean(data$value[1:12])
  expect_equal(baseline_mean, 10,
    tolerance = 0.01,
  ...
  expect_equal(cl_values[1], baseline_mean,
    tolerance = 0.1,
    label = "Frozen CL = baseline mean"
  )
  expect_equal(cl_values[24], baseline_mean,
    tolerance = 0.1,
...
test_that("part-parameter producerer separate CL'er pr. phase", {
  ...
  result <- bfh_qic(data, x = period, y = value, chart_type = "i", part = 10)
  ...
  expect_equal(unique(cl_phase_1), 10,
    tolerance = 0.01,
    label = "Phase 1 CL"
  )
  expect_equal(unique(cl_phase_2), 20,
    tolerance = 0.01,
    label = "Phase 2 CL"
  )
...
test_that("centerlinje-beregning er konsistent for kendte datasets", {
  ...
  # c-chart (mean = 25)
  result_c <- bfh_qic(constant_data, x = period, y = value, chart_type = "c")
  expect_equal(result_c$qic_data$cl[1], 25, tolerance = 1e-6)

  # i-chart (mean = 25)
  result_i <- bfh_qic(constant_data, x = period, y = value, chart_type = "i")
  expect_equal(result_i$qic_data$cl[1], 25, tolerance = 1e-6)

  # run-chart (median = 25)
  result_run <- bfh_qic(constant_data, x = period, y = value, chart_type = "run")
...
test_that("run-chart centerlinje er median (ikke mean)", {
  ...
  expect_valid_bfh_qic_result(result)
  expect_equal(result$qic_data$cl[1], 3,
    tolerance = 1e-6,
