...
**Files to modify:**
- `DESCRIPTION` - Remove VignetteBuilder line

**Conditional changes:**
- Review `knitr` in Suggests - remove if only for vignettes
- Review `rmarkdown` in Suggests - remove if only for vignettes

**Check for usage:**
```r
# In R console, verify no code uses knitr/rmarkdown:
grep -r "knitr::" R/
grep -r "rmarkdown::" R/
```
...
**Before change:**
- R CMD CHECK shows: "VignetteBuilder specified but no vignettes found"

**After change:**
- R CMD CHECK shows no vignette-related messages
- Package configuration is consistent
...
**When to re-add VignetteBuilder:**
- Public release decision made
- Vignettes fully implemented
- pkgdown website planned (issue #17)
