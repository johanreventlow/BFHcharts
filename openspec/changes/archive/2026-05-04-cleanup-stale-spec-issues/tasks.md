...
- [x] 1.1 Inspicer `R/cache_marquee_styles.R` for at dokumentere
      ...
      antagelser).
- [x] 1.2 Opdater `openspec/specs/caching-system/spec.md`:
...
- [x] 2.1 Inspicer `R/utils_add_right_labels_marquee.R` for at
...
- [x] 3.1 Identificer overlap mellem `public-api` og `spc-analysis-api`:
...
- [x] 3.2 Opdater `public-api` Purpose: "ejer af user-facing API
      contracts (signaturer, eksport-status, return-types,
      attribute-existence)".
- [x] 3.3 Opdater `spc-analysis-api` Purpose: "ejer af internal
      signal-detection logic (Anhoej rules, fallback-narrative
      dispatch, threshold semantics)".
- [x] 3.4 Fjern duplikeret indhold:
...
- [x] 4.1 `openspec validate cleanup-stale-spec-issues --strict` skal
      vaere green.
- [x] 4.2 Sync delta-specs til main specs (haandholdt eller via
      openspec-sync-specs skill).
- [x] 4.3 `openspec validate --specs --strict` skal vaere 7/7
      green efter sync.

## 5. Documentation

- [x] 5.1 NEWS.md (development): tilfoej kort entry under
      `## Interne aendringer`:
      ...
      ejerskab."
- [x] 5.2 Ingen ADR -- aendringer er rent dokumentations-cleanup uden
      arkitekturelle beslutninger der kraever permanent record.

## 6. Out-of-scope follow-up

- [x] 6.1 Phase 1 #14 (pdf-smoke.yaml comment refresh) -- pre-tool-use
      hook blokerer; manuel opgave for maintainer.
- [x] 6.2 Verify implementation matches new code-organization #7
