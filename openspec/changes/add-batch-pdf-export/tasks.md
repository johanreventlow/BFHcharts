...
- [ ] 1.1 Extract per-page parameter rendering from `build_typst_content()`
...
- [ ] 1.2 Generalize workspace/template staging so a batch workspace can stage
...
- [ ] 2.1 Add `BATCH_CACHE_FORMAT_VERSION <- 1L` to `R/globals.R` and create
...
- [ ] 2.2 Implement id validation (`^[A-Za-z0-9][A-Za-z0-9_-]*$`), default id
      ...
      id/order sequencing.
- [ ] 2.3 Implement atomic stage-and-rename (`.staging-<id>-<pid>` → bundle
...
- [ ] 2.4 Input validation errors as classed `bfhcharts_export_error` for
...
- [ ] 3.1 Implement bundle discovery + validation: read `page.rds` files,
      ...
      skipped ids.
- [ ] 3.2 Implement batch document composition: stage template once, copy
...
- [ ] 3.3 Wire compilation through `bfh_compile_typst()` with
...
- [ ] 4.1 Rendering-equivalence check: stage one chart and batch-compile vs
...
- [ ] 4.2 Font-embedding assertion: batch-compile >= 2 pages and verify via
      ...
      tooling.
- [ ] 4.3 Scale benchmark (manual/nightly, not PR-blocking): synthetic
